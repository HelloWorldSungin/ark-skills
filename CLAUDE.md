...
- **T2:** MemPalace MCP server reachable (`mcp__mempalace__*` tools available) for reads; `mempalace` CLI installed for ingest (`mempalace mine`, requires **v3.3.5+**). The HNSW-corruption crash class — `_query` ([#1132](https://github.com/MemPalace/mempalace/issues/1132)) and `_upsert` ([#976](https://github.com/MemPalace/mempalace/pull/976)) — is closed in v3.3.5 via [#1322](https://github.com/MemPalace/mempalace/pull/1322) (wires `quarantine_stale_hnsw` into the chromadb client open path). v3.3.5 also ships [`mempalace repair --mode from-sqlite`](https://github.com/MemPalace/mempalace/pull/1310) for recovering already-corrupt palaces (reads `(id, document, metadata)` directly from `chroma.sqlite3` without opening chromadb against the corrupt palace). **Caveat:** [#1457](https://github.com/MemPalace/mempalace/issues/1457) (P1, open) — the quarantine gate misses zero-byte `link_lists.bin` and can still SIGSEGV; manual segment `mv` workaround still applies until the follow-up lands. `mempalace status` hits SQLite's 32k-variable limit on palaces past ~32k drawers ([#802](https://github.com/MemPalace/mempalace/issues/802)). If MCP is unreachable, skip T2 entirely.
...
- "T2 not available — MemPalace MCP server unreachable. CLI search/mine require v3.3.5+ (#1132/#976 closed via #1322); existing on-disk HNSW bloat may still need manual repair (#1457). Skipping T2. Falling back to T3/T4."
