...
Neither `CONTEXT.md` nor `docs/adr/` exists yet in this repo. If any of
these files don't exist, **proceed silently**. Don't flag their absence;
...
Single-context repo layout (this repo is single-context — `ark-skills` is
one plugin, not a monorepo with separate contexts):

```
/
├── CONTEXT.md          (not yet created — lazy)
...
`CONTEXT.md` once it exists. Don't drift to synonyms the glossary
explicitly avoids.
