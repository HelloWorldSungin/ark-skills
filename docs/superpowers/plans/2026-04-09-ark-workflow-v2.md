# Ark Workflow v2 Implementation Plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** Rewrite `skills/ark-workflow/SKILL.md` to address 12 identified gaps (plus 10 from Codex review) and add continuity mechanisms for in-session and cross-session chain tracking.

**Architecture:** Single-file markdown rewrite. The skill is a router document that triages tasks into scenarios (7 total) and weight classes, then outputs resolved skill chains. Each phase rewrites a contiguous section of the file. After each phase, verify with grep and a read-through.

**Tech Stack:** Markdown with YAML frontmatter. Bash for Project Discovery detection. TodoWrite + filesystem (`.ark-workflow/current-chain.md`) for continuity.

**Spec:** `docs/superpowers/specs/2026-04-09-ark-workflow-v2-design.md` (944 lines)
**Current file:** `skills/ark-workflow/SKILL.md` (391 lines → ~700 lines after rewrite)

---

## Execution Notes

- **No code tests exist for this skill** (it's markdown). Instead, each task has a "verify" step using grep, read, or structural checks.
- **Commit after each phase**, not each step. Phases are sized to match one commit each.
- **Reference the spec by section** to avoid duplicating content in this plan. The spec is the source of truth for target content.
- **Re-read the current skill** at the start of each phase (Edit Integrity rule from CLAUDE.md).
- Phases touch ONE file: `skills/ark-workflow/SKILL.md`. Auxiliary `.gitignore` change in Phase 5.

---

## Phase 1: Preparation + Frontmatter

**Files:**
- Read: `skills/ark-workflow/SKILL.md` (full file, 391 lines)
- Read: `docs/superpowers/specs/2026-04-09-ark-workflow-v2-design.md` (sections 1-4)
- Modify: `skills/ark-workflow/SKILL.md` (frontmatter only in this phase)

### Task 1.1: Read current skill and spec

- [ ] **Step 1: Read the full current skill**

Use the Read tool on `skills/ark-workflow/SKILL.md`. Note the current structure so you know what's being replaced.

- [ ] **Step 2: Read the spec sections 1-4**

Use the Read tool on `docs/superpowers/specs/2026-04-09-ark-workflow-v2-design.md`. Focus on:
- Section "Frontmatter" (spec line ~43-57)
- Section "Section 1: Header + Purpose" (spec line ~59-61)
- Section "Section 2: Project Discovery" (spec line ~63-73)
- Section "Section 3: Scenario Detection" (spec line ~75-103)
- Section "Section 4: Triage" (spec line ~104-167)

- [ ] **Step 3: Verify spec file hash for reference**

Run: `shasum docs/superpowers/specs/2026-04-09-ark-workflow-v2-design.md`
Record the hash in your notes for traceability.

### Task 1.2: Rewrite frontmatter

**Files:**
- Modify: `skills/ark-workflow/SKILL.md:1-4` (YAML frontmatter)

- [ ] **Step 1: Replace the frontmatter**

Replace lines 1-4 of `skills/ark-workflow/SKILL.md` with the new frontmatter from the spec. Use the Edit tool:
...
Run: `head -5 skills/ark-workflow/SKILL.md`
Expected: Shows the new description with "upgrade", "migrate", "slow", "optimize", "benchmark" in the trigger list.

### Task 1.3: Add early-exit logic to Project Discovery

The current Project Discovery section ends at step 7 (HAS_VAULT notice). The spec requires a new step 8 that stops triage early when Knowledge Capture + no vault. Add it now so later sections can reference it.

**Files:**
- Modify: `skills/ark-workflow/SKILL.md` — the numbered list inside `## Project Discovery`

- [ ] **Step 1: Append Step 8 to Project Discovery**

Use the Edit tool. Find this exact text (currently the last numbered item in Project Discovery):
...
Run: `grep -c "Early exits" skills/ark-workflow/SKILL.md`
Expected: 1
...
git commit -m "refactor(ark-workflow): update frontmatter and add early-exit logic to Project Discovery"
```

**CHECKPOINT** — If a user is reviewing interactively (default for the ark-skills repo per CLAUDE.md "PHASED EXECUTION" rule), stop and get user approval before Phase 2. If the user has granted autonomous execution, proceed directly.

---

## Phase 2: Scenario Detection + Triage (Sections 3-4)

**Files:**
- Read: `skills/ark-workflow/SKILL.md` (re-read to refresh context)
- Modify: `skills/ark-workflow/SKILL.md:55-97` (Scenario Detection + Triage sections)

### Task 2.1: Re-read current skill

- [ ] **Step 1: Re-read the file**

Use the Read tool on `skills/ark-workflow/SKILL.md`. Context decay may have occurred since Phase 1.

### Task 2.2: Replace Scenario Detection section

**Files:**
- Modify: `skills/ark-workflow/SKILL.md` — the "## Scenario Detection" section and its table

- [ ] **Step 1: Replace scenario detection**

Use the Edit tool to replace the entire "## Scenario Detection" section (from `## Scenario Detection` through the end of the disambiguation question, before `## Triage`). New content:
...
Run: `grep -c "Migration" skills/ark-workflow/SKILL.md`
Expected: 2 (scenario row + disambiguation option F)

Run: `grep -c "Audit-Only" skills/ark-workflow/SKILL.md`
Expected: At least 1

### Task 2.3: Replace Triage section

**Files:**
- Modify: `skills/ark-workflow/SKILL.md` — the "## Triage" section

- [ ] **Step 1: Replace triage section**

Use the Edit tool to replace the entire "## Triage" section (from `## Triage` through the end of the re-triage rule, before `## Workflow`). New content:
...
- [ ] **Step 2: Verify triage section**

Run: `grep -c "decision density" skills/ark-workflow/SKILL.md`
Expected: At least 3

Run: `grep -c "risk floor\|risk as floor\|escalate" skills/ark-workflow/SKILL.md`
Expected: At least 2
...
```bash
git add skills/ark-workflow/SKILL.md
git commit -m "refactor(ark-workflow): expand scenarios to 7, rewrite triage as risk-primary with decision-density escalation"
```

**CHECKPOINT** — If a user is reviewing interactively, stop and get user approval before Phase 3. If running autonomously, proceed directly.

---

## Phase 3: Batch Triage + Continuity Section

**Files:**
- Read: `skills/ark-workflow/SKILL.md` (re-read)
- Read: `docs/superpowers/specs/2026-04-09-ark-workflow-v2-design.md` (Section 5 + 5.5)
- Modify: `skills/ark-workflow/SKILL.md` — insert Batch Triage + Continuity sections

### Task 3.1: Re-read files

- [ ] **Step 1: Re-read current skill**

Use the Read tool.

- [ ] **Step 2: Re-read spec Section 5 (Batch Triage) and Section 5.5 (Continuity)**

Spec lines approximately 135-300.

### Task 3.2: Insert Batch Triage section

**Files:**
- Modify: `skills/ark-workflow/SKILL.md` — insert new section after "## Triage", before "## Workflow"

- [ ] **Step 1: Insert Batch Triage**

Use the Edit tool. Find the line `## Workflow` and insert the following BEFORE it. The content to insert is delimited by `=====BEGIN=====` and `=====END=====` markers — insert everything between them verbatim (do NOT include the markers themselves):
...
- [ ] **Step 2: Verify Batch Triage**

Run: `grep -c "Batch Triage\|Root cause consolidation\|heuristic based on item" skills/ark-workflow/SKILL.md`
Expected: At least 3

### Task 3.3: Insert Continuity section

**Files:**
- Modify: `skills/ark-workflow/SKILL.md` — insert new section after "## Batch Triage", before "## Workflow"

- [ ] **Step 1: Insert Continuity section**

Use the Edit tool. Find the line `## Workflow` and insert the content between the `=====BEGIN=====` and `=====END=====` markers below (do NOT include the markers themselves):
...
- [ ] **Step 2: Verify Continuity section**

Run: `grep -c "current-chain.md\|handoff_marker\|Rehydrate" skills/ark-workflow/SKILL.md`
Expected: At least 5
...
```bash
git add skills/ark-workflow/SKILL.md
git commit -m "refactor(ark-workflow): add Batch Triage and Continuity sections"
```

**CHECKPOINT** — If a user is reviewing interactively, stop and get user approval before Phase 4. If running autonomously, proceed directly.

---

## Phase 4: Workflow Steps + Skill Chains Rewrite

This phase rewrites the Workflow Steps (adding Step 6.5) and replaces all existing chain sections with the updated versions including the new scenarios (Migration, Performance) and updated chains (`/TDD` → `/test-driven-development`, `/investigate` added to Hygiene, Knowledge Capture Light/Full split, Hygiene Audit-Only variant).

**Files:**
- Read: `skills/ark-workflow/SKILL.md` (re-read)
- Read: `docs/superpowers/specs/2026-04-09-ark-workflow-v2-design.md` (Section 6-7)
- Modify: `skills/ark-workflow/SKILL.md` — Workflow section + Skill Chains section

### Task 4.1: Re-read files

- [ ] **Step 1: Re-read current skill**

Use the Read tool.

- [ ] **Step 2: Re-read spec Section 6 (Workflow Steps) and Section 7 (Skill Chains)**

### Task 4.2: Replace Workflow Steps section

**Files:**
- Modify: `skills/ark-workflow/SKILL.md` — the "## Workflow" section

- [ ] **Step 1: Replace Workflow section**

Use the Edit tool to replace the entire "## Workflow" section (from `## Workflow` through the end of `### Step 7: Hand Off` block, before `## Skill Chains`). New content:
...
- [ ] **Step 2: Verify Workflow section**

Run: `grep -c "Step 6.5\|decision-density\|dedup rule" skills/ark-workflow/SKILL.md`
Expected: At least 3

### Task 4.3: Replace Skill Chains section — Greenfield, Bugfix, Ship

**Files:**
- Modify: `skills/ark-workflow/SKILL.md` — from `## Skill Chains` through end of Bugfix chains

- [ ] **Step 1: Replace Greenfield + Bugfix + Ship chains**

Use the Edit tool. Replace from `## Skill Chains` through the end of the "### Shipping & Deploying" section (before `### Knowledge Capture`). New content:
...
- [ ] **Step 2: Verify no `/TDD` remaining in Greenfield/Bugfix chains**

Run: `grep -c "/TDD" skills/ark-workflow/SKILL.md`
Expected: 0 (or at most references in the Hygiene/Migration/Performance sections not yet replaced)

Run: `grep -c "/test-driven-development" skills/ark-workflow/SKILL.md`
Expected: At least 4

### Task 4.4: Replace Knowledge Capture + Hygiene chains

- [ ] **Step 1: Replace Knowledge Capture section**

Use the Edit tool to replace the entire "### Knowledge Capture" section. New content:
...
- [ ] **Step 2: Replace Hygiene section (add Audit-Only, update L/M/H)**

Use the Edit tool to replace the entire "### Codebase Hygiene" section. New content:
...
Run: `grep -c "Audit-Only\|Dedup rule\|Light/Full" skills/ark-workflow/SKILL.md`
Expected: At least 2

Run: `grep -c "/TDD" skills/ark-workflow/SKILL.md`
Expected: 0 (all instances should now be `/test-driven-development`)

### Task 4.5: Add Migration chains

**Files:**
- Modify: `skills/ark-workflow/SKILL.md` — insert new Migration section after Hygiene

- [ ] **Step 1: Insert Migration section**

Use the Edit tool. Find the end of the Hygiene section (after the Dedup rule line and its `---` separator) and insert before the `## Condition Resolution` section:
...
- [ ] **Step 2: Verify Migration section**

Run: `grep -c "### Migration" skills/ark-workflow/SKILL.md`
Expected: 1

Run: `grep -c "mandatory for Heavy migrations" skills/ark-workflow/SKILL.md`
Expected: 1

### Task 4.6: Add Performance chains

**Files:**
- Modify: `skills/ark-workflow/SKILL.md` — insert new Performance section after Migration

- [ ] **Step 1: Insert Performance section**

Use the Edit tool. Find the end of the Migration section and insert before `## Condition Resolution`:
...
- [ ] **Step 2: Verify Performance section and all 19 chains**

Run: `grep -c "### Performance\|### Migration\|### Greenfield\|### Bug Investigation\|### Shipping\|### Knowledge\|### Codebase" skills/ark-workflow/SKILL.md`
Expected: 7

Run: `grep -c "^\*\*Light\|^\*\*Medium\|^\*\*Heavy\|^\*\*Audit-Only\|^\*\*Full" skills/ark-workflow/SKILL.md`
Expected: At least 18 (some may vary by exact formatting)
...
```bash
git add skills/ark-workflow/SKILL.md
git commit -m "refactor(ark-workflow): rewrite chains for 7 scenarios, add Migration/Performance, Knowledge Capture Light/Full, Hygiene Audit-Only"
```

**CHECKPOINT** — If a user is reviewing interactively, stop and get user approval before Phase 5. If running autonomously, proceed directly.

---

## Phase 5: Condition Resolution, Session Handoff, Re-triage, When Things Go Wrong, Routing Rules

**Files:**
- Read: `skills/ark-workflow/SKILL.md` (re-read)
- Read: `docs/superpowers/specs/2026-04-09-ark-workflow-v2-design.md` (Sections 8-12)
- Modify: `skills/ark-workflow/SKILL.md` — remaining sections
- Create: `.gitignore` entry for `.ark-workflow/`

### Task 5.1: Re-read files

- [ ] **Step 1: Re-read current skill**

Use the Read tool.

### Task 5.2: Replace Condition Resolution section

- [ ] **Step 1: Replace Condition Resolution**

Use the Edit tool to replace the entire "## Condition Resolution" section. New content:
...
- [ ] **Step 2: Verify Condition Resolution**

Run: `grep -c "Investigation triggers\|both read AND write\|Database schema changes" skills/ark-workflow/SKILL.md`
Expected: At least 3

### Task 5.3: Replace Session Handoff section

- [ ] **Step 1: Replace Session Handoff**

Use the Edit tool to replace the entire "## Session Handoff" section. New content:
...
Run: `grep -c "handoff_marker\|unreliable proxy" skills/ark-workflow/SKILL.md`
Expected: At least 2

### Task 5.4: Replace Re-triage + When Things Go Wrong sections

- [ ] **Step 1: Replace Re-triage**

Use the Edit tool to replace the entire "## Re-triage" section. New content:
...
- [ ] **Step 2: Replace When Things Go Wrong**

Use the Edit tool to replace the entire "## When Things Go Wrong" section. New content:
...
Run: `grep -c "Scenario shift\|Hygiene reveals bugs\|Batch item blocks" skills/ark-workflow/SKILL.md`
Expected: At least 3

### Task 5.5: Replace Routing Rules Template

- [ ] **Step 1: Replace Routing Rules Template section**

Use the Edit tool to replace the entire "## Routing Rules Template" section. New content:
...
Run: `grep -c "Session start — check for in-progress chain\|After each step in a running chain" skills/ark-workflow/SKILL.md`
Expected: 2

### Task 5.6: Add `.ark-workflow/` to the ark-skills project's own .gitignore

**Files:**
- Modify: `.gitignore` at ark-skills project root

- [ ] **Step 1: Check current .gitignore**

Run: `cat .gitignore`

- [ ] **Step 2: Append if missing**

If `.ark-workflow/` is not already in `.gitignore`, use the Edit tool to append it. If `.gitignore` doesn't exist, create it with:

```
# Ark workflow chain state (per-project ongoing work)
.ark-workflow/
```

If `.gitignore` exists, add the two lines at the end.

- [ ] **Step 3: Verify**

Run: `grep -c "\.ark-workflow" .gitignore`
Expected: 1
...
```bash
git add skills/ark-workflow/SKILL.md .gitignore
git commit -m "refactor(ark-workflow): rewrite Condition Resolution, Session Handoff, Re-triage, Routing Rules; add .ark-workflow gitignore"
```

**CHECKPOINT** — If a user is reviewing interactively, stop and get user approval before Phase 6. If running autonomously, proceed directly.

---

## Phase 6: Verification, Smoke Tests, Version Bump

**Files:**
- Read-only: `skills/ark-workflow/SKILL.md` for all verification tasks
- Modify: `VERSION`, `.claude-plugin/plugin.json`, `.claude-plugin/marketplace.json`, `CHANGELOG.md` (in Task 6.5 only)

### Task 6.1: Structural checks

- [ ] **Step 1: Verify section headers exist**

Run: `grep -c "^## Scenario Detection$\|^## Triage$\|^## Batch Triage$\|^## Continuity\|^## Workflow$\|^## Skill Chains$\|^## Condition Resolution$\|^## Session Handoff$\|^## Re-triage$\|^## When Things Go Wrong$\|^## Routing Rules Template$" skills/ark-workflow/SKILL.md`
Expected: 11

- [ ] **Step 2: Verify no `/TDD` remains**

Run: `grep -c "/TDD" skills/ark-workflow/SKILL.md`
Expected: 0

- [ ] **Step 3: Count `/test-driven-development` references**

Run: `grep -c "/test-driven-development" skills/ark-workflow/SKILL.md`
Expected: At least 7 (one per chain that uses TDD)

- [ ] **Step 4: Verify 7 scenario chains exist**

Run: `grep -c "^### Greenfield Feature$\|^### Bug Investigation & Fix$\|^### Shipping & Deploying$\|^### Knowledge Capture$\|^### Codebase Hygiene$\|^### Migration$\|^### Performance$" skills/ark-workflow/SKILL.md`
Expected: 7

- [ ] **Step 5: Verify 19 chain variants**

Count weight classes across all 7 scenarios:
- Greenfield L/M/H = 3
- Bugfix L/M/H = 3
- Ship = 1
- Knowledge Capture Light/Full = 2
- Hygiene Audit-Only/L/M/H = 4
- Migration L/M/H = 3
- Performance L/M/H = 3
- Total: 19

Run: `grep -cE "^\*\*(Light|Medium|Heavy|Audit-Only|Full)" skills/ark-workflow/SKILL.md`
Expected: 18 (Ship has no weight class bold header, but has its own section — count is 19 if you include Ship, 18 by bold weight headers)

### Task 6.2: Content checks

- [ ] **Step 1: Investigation triggers documented**

Run: `grep -c "Investigation triggers" skills/ark-workflow/SKILL.md`
Expected: At least 1

- [ ] **Step 2: Continuity mechanism documented**

Run: `grep -c "current-chain.md\|handoff_marker\|Rehydrate" skills/ark-workflow/SKILL.md`
Expected: At least 5

- [ ] **Step 3: Security routing split documented**

Run: `grep -c "Audit-Only\|Dedup rule\|security audit.*hardening\|security hardening.*audit" skills/ark-workflow/SKILL.md`
Expected: At least 3

- [ ] **Step 4: Risk+density triage documented**

Run: `grep -c "decision density\|risk.*floor\|Escalat" skills/ark-workflow/SKILL.md`
Expected: At least 4

### Task 6.3: Full read-through

- [ ] **Step 1: Read the entire new skill.md end-to-end**

Use the Read tool on `skills/ark-workflow/SKILL.md`. Verify:
- No duplicate sections
- No orphaned references to removed content
- Numbered lists are correctly numbered
- Tables render correctly (pipes aligned, no broken rows)
- Code blocks are properly closed

### Task 6.4: Smoke tests (mental walkthroughs)

- [ ] **Step 1: Smoke test Session A (Data & Orchestrator)**

Mentally run the Session A prompt from the spec (5 items: Transaction isolation, Ghost pipeline runs, Payload drop, Retry storms, MCP ClosedResourceError) through the new skill. Expected triage:
- All 5 are Bugfix scenario (not Hygiene)
- Weight classes: 1=Medium, 2=Light, 3=Medium, 4=Heavy, 5=Light
- Batch Triage is triggered because 5 numbered items
- Group C (Heavy) should be suggested for a separate session

Verify your reading of the skill produces this triage.

- [ ] **Step 2: Smoke test Session B (Dashboard & Infra)**

Expected:
- 1 File permissions → Bugfix Heavy
- 2 SSE tunnel gap → Bugfix Medium
- 3 Dashboard blocking calls → Performance Light
- Mixed scenarios; Performance item routes correctly

- [ ] **Step 3: Smoke test Session C (CI & Cleanup)**

Expected:
- 1 Deploy script TS build → Bugfix Medium (broken behavior)
- 2 ESLint cleanup → Hygiene Light (pure cleanup)

- [ ] **Step 4: Smoke test Security Audit**

Prompt: "Run a security audit on our auth module."
Expected:
- Scenario: Hygiene Audit-Only
- Chain: `/cso` → findings report → `/wiki-update` → STOP
- No implementation, no ship

- [ ] **Step 5: Smoke test Security Hardening**

Prompt: "Harden our auth module against session hijacking."
Expected:
- Scenario: Hygiene Heavy with mandatory early `/cso`
- Chain: `/cso` (step 1) → `/codebase-maintenance` → `/investigate` → ~~`/cso`~~ (deduped) → ...
- `/cso` runs exactly once

- [ ] **Step 6: Smoke test Decision-Density Escalation**

Prompt: "Redesign the internal caching layer — rewrite how we cache DB query results."
Expected:
- Risk: Low (internal only)
- Decision density: Architecture
- Weight: Heavy (escalated)
- Chain: Heavy Hygiene OR Heavy Greenfield (depending on interpretation)

- [ ] **Step 7: Smoke test Cross-Session Resume**

Mental walkthrough: Simulate ending a session mid-Heavy-Greenfield (step 4 of 21), then opening a new session.
Expected behavior:
- New session should check `.ark-workflow/current-chain.md`
- Read the frontmatter and step list
- Announce: "Found an in-progress ark-workflow chain: Greenfield/Heavy, step 4 of 21 complete. Next: ..."
- Rehydrate TodoWrite tasks for unchecked steps
- Resume from step 5

### Task 6.5: Version bump + final commit

Per the user's feedback memory: "Always bump plugin version — Bump VERSION, `.claude-plugin/plugin.json`, `.claude-plugin/marketplace.json`, CHANGELOG on every push to master."

**Files:**
- Modify: `VERSION` (at repo root)
- Modify: `.claude-plugin/plugin.json` (NOT at repo root — under `.claude-plugin/`)
- Modify: `.claude-plugin/marketplace.json` (also under `.claude-plugin/`)
- Modify: `CHANGELOG.md` (at repo root)

- [ ] **Step 1: Check current versions**

Run: `cat VERSION && grep version .claude-plugin/plugin.json && grep version .claude-plugin/marketplace.json | head -3`

- [ ] **Step 2: Bump minor version**

This is a feature-level rewrite with new scenarios and continuity mechanisms. Bump the MINOR version (e.g., 1.4.2 → 1.5.0). Use the Edit tool on each file:
- `VERSION` — replace the version string
- `.claude-plugin/plugin.json` — update the `"version"` field
- `.claude-plugin/marketplace.json` — update the `"version"` field (there may be multiple — check the JSON structure carefully)
...
- **ark-workflow**: Major rewrite of the task triage skill addressing 22 gaps (12 initial + 10 from Codex review)
  ...
  - Replaced risk-only triage with risk-primary + decision-density escalation
  ...
  - Added Continuity mechanism: TodoWrite tasks + `.ark-workflow/current-chain.md` state file for in-session and cross-session chain tracking
  - Added Hygiene Audit-Only variant for assessment-only requests (no implementation/ship forced)
  - Split security routing into Audit and Hardening paths, with `/cso` dedup rule
  ...
  - Added scenario-shift re-triage handling
  - Fixed `/TDD` naming to `/test-driven-development` (7 occurrences)

### Fixed
- ark-workflow: removed unreliable "10 tool calls since last file read" handoff trigger
...
```bash
git add skills/ark-workflow/SKILL.md VERSION .claude-plugin/plugin.json .claude-plugin/marketplace.json CHANGELOG.md
git commit -m "release: bump to 1.5.0 — ark-workflow v2 rewrite with batch triage, continuity, 7 scenarios"
```
...
1. ✅ `skills/ark-workflow/SKILL.md` is fully rewritten per the spec
2. ✅ All 19 chain variants exist and use `/test-driven-development` (not `/TDD`)
3. ✅ 7 scenarios are in the detection table (including Migration and Performance)
4. ✅ Triage uses risk-primary + decision-density escalation
5. ✅ Batch Triage section with root cause consolidation, dependency heuristics, grouping
6. ✅ Continuity section with in-session TodoWrite + cross-session chain state file + rehydration
7. ✅ Security routing split (Audit vs Hardening) with `/cso` dedup rule
8. ✅ Hygiene Audit-Only variant
9. ✅ `/investigate` added to Hygiene chains as conditional
10. ✅ Session handoff expanded to cover Heavy Bugfix/Hygiene/Migration/Performance
11. ✅ Re-triage covers both weight and scenario shifts
12. ✅ Routing Rules Template includes session-resume block
13. ✅ `.gitignore` includes `.ark-workflow/`
14. ✅ All smoke tests pass (Sessions A/B/C + security audit/hardening + escalation + cross-session)
15. ✅ Version bumped, CHANGELOG updated
...
**Spec coverage:**
- Frontmatter → Phase 1 ✓
- Project Discovery → unchanged, Phase 1 verification ✓
- Scenario Detection (7 scenarios) → Phase 2 ✓
- Triage (risk-primary + density) → Phase 2 ✓
- Batch Triage → Phase 3 ✓
- Continuity → Phase 3 ✓
- Workflow Steps + Step 6.5 → Phase 4 ✓
- All 7 skill chain sections → Phase 4 ✓
- Condition Resolution → Phase 5 ✓
- Session Handoff → Phase 5 ✓
- Re-triage → Phase 5 ✓
- When Things Go Wrong → Phase 5 ✓
- Routing Rules Template → Phase 5 ✓
- `.gitignore` update → Phase 5 ✓
- Verification → Phase 6 ✓

**Placeholder scan:** No TBDs, no "add appropriate error handling", no "similar to Task N", no "TDD tests for the above" without actual test code. All content is embedded in the plan.

**Type consistency:** Skill names (`/test-driven-development`, `/investigate`, `/cso`, etc.) are consistent across all phases. Chain section names match between the Scenario Detection table and the Skill Chains section.
