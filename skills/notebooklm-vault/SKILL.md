...
description: Persistent context and memory layer backed by the Obsidian vault synced to Google NotebookLM. Use this skill whenever starting a fresh session and needing project history, when asking "what happened in session X", "what's the current project state", "has this been tried before", when ending a session and creating a handoff document, or when generating audio/reports from vault content. Triggers on bootstrap, vault context, session history, session handoff, conflict check, or any question about past decisions/experiments. Also triggers on "wrap up", "end session", "hand off", "what's next", "session log", "continue", "pick up where I left off", or "resume". Do NOT use for general NotebookLM operations unrelated to the vault — use the global notebooklm skill for those.
---
...
### `session-handoff` — End-of-Session Log + Sync + TaskNote Update

Performs three steps in sequence: writes the session log to the vault, syncs it to NotebookLM, and updates the linked TaskNote epic and stories.

#### Step 1: Create or Update the Session Log

Use the `obsidian:obsidian-markdown` skill for proper Obsidian-flavored markdown (wikilinks, callouts, frontmatter).

1. Find the highest-numbered `Session-NNN.md` in `{project_docs_path}/Session-Logs/` (use Glob, then sort by numeric session number).
2. Read its `epic` field via `obsidian` CLI:
   ```bash
   obsidian property:read name="epic" file="Session-<NNN>"
   ```
3. Determine whether to **create** or **update**:
   - **If a session log was created during this conversation** -> update it with final results.
   - **If the last session log's `epic` matches the current work's epic** -> update that session log by appending a continuation section.
   - **Otherwise** -> create a new `Session-<N+1>.md`.
4. Identify the related TaskNote epic:
   - If the user mentioned an epic (e.g., "{task_prefix}001"), use that.
   - If the work clearly relates to an existing epic, find it in `{tasknotes_path}/Tasks/Epic/`.
   - If the work is a bug fix, check `{tasknotes_path}/Tasks/Bug/` for an existing bug ticket or create one.
   - If no epic exists for this work and the work is substantial enough, create one (see Step 3).
   - If the work is minor/ad-hoc with no epic, set `epic: ""`.

**Session log structure (new session):**
```markdown
---
date: YYYY-MM-DD
tags:
...
## Epic
[[<epic-id>-<slug>]] — <epic title>

## Objective
[What this session set out to accomplish]

## Implementation / Work Done
[Key changes, files modified, commands run, decisions made]

## Results
[Experiment outcomes, metrics, analysis results]

## Issues & Discoveries
[Bugs found, gotchas, unexpected behaviors]

## Next Steps
[Actionable items for the next session, ordered by priority]
```

**Continuation format (appending to existing session log):**
```markdown

---

## Continuation — YYYY-MM-DD — <Optional Brief Title>

### Objective
[What this continuation session set out to accomplish]

### Work Done
[Additional changes, files modified, decisions made]

### Results
[New outcomes, metrics]
...
5. Ensure all experiment results, discoveries, and architectural decisions from the conversation are captured.

#### Step 2: Notify sync status

the scheduled sync service

To verify the session log was picked up, use `/notebooklm-vault status` after ~15 minutes.

#### Step 3: Update TaskNote Epic and Stories

After the session log is synced, update the project management layer.

1. **Find or create the epic:**
   - Search for the epic using `obsidian` CLI:
     ```bash
     obsidian read file="<epic-id>-<slug>"
     ```
   - If the work warrants a new epic and none exists:
     - Read the next ID from `{tasknotes_path}/meta/{task_prefix}counter`.
     - Verify it doesn't collide with existing tasks. Task IDs must be globally unique.
     - Increment the counter and write it back.
     - Create the epic in `Tasks/Epic/<id>-<slug>.md` with standard frontmatter.
     - For bugs, create in `Tasks/Bug/<id>-<slug>.md` instead.

2. **Update the epic with session progress:**
   ```bash
   obsidian property:set name="session" value="<NNN>" file="<epic-id>-<slug>" silent
   ```
   Append a session log entry to the epic body:
   ```bash
   obsidian append file="<epic-id>-<slug>" content="\n- [[Session-<NNN>]] — YYYY-MM-DD: <one-line summary>" silent
   ```
...
3. **Update related stories:**
   ...
   ```
   For each story returned by backlinks:
   a. Read the story's current status: `obsidian property:read name="status" file="<story-id>"`
   b. Determine if the story was worked on in this session by checking:
      - Does the session log mention this story's topic/feature?
      - Were files related to this story changed on the branch? (cross-reference `git diff --name-only`)
   c. If the story was worked on, update its status based on the session's outcome:
      - Work completed and verified → `done`
      - Work started but not finished → `in-progress`
      - Work blocked by an issue → `blocked`
   d. Update the story's session reference:
      ```bash
      obsidian property:set name="session" value="<NNN>" file="<story-id>" silent
      ```

4. **Check epic completion and set epic status:**
...
**TaskNote frontmatter conventions:**
```yaml
---
tags:
  - task
title: "<descriptive title>"
task-id: "{task_prefix}<NNN>"
status: backlog | todo | in-progress | blocked | review | done
priority: critical | high | medium | low
project: "{project_name}"
work-type: research | development | infrastructure | documentation
task-type: epic | story | task | bug
urgency: normal | blocking | high | low
